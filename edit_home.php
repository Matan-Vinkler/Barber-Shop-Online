        ...
        <div class="container"><button data-bs-toggle="collapse" class="navbar-toggler" data-bs-target="#navcol-1"><span class="visually-hidden">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
            <div class="collapse navbar-collapse" id="navcol-1">
                ...
                <ul class="navbar-nav mx-auto">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="/edit_home.php">Edit Home</a></li>
                    ...
                    <div class="card">
                        <div class="card-body text-center d-flex flex-column align-items-center">
                            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype="multipart/form-data">
                                <div class="mb-3"><textarea class="form-control" name="about" rows="5" cols="33" placeholder="Edit About Section..."></textarea></div>
                                <div class="mb-3"><input class="form-control" type="file" name="image_board" accept="image/png"></div>
                            ...
                            <?php    
                                ...
                                if($_SERVER["REQUEST_METHOD"] == "POST") {
                                    ...
                                    $imageBoard = $_FILES["image_board"];

                                    if($imageBoard["size"] == 0 or empty($about)) {
                                        echo "File cannot be empty";
                                        die();
                                    }

                                    $imageFileType = strtolower(pathinfo($imageBoard["name"],PATHINFO_EXTENSION));
                                    if($imageFileType != "png") {
                                        echo "Sorry, only PNG files are allowed.";
                                        die();
                                    }

                                    if (!move_uploaded_file($imageBoard["tmp_name"], "assets/img/deals.png")) {
                                    ...
                                    $sql_query = sprintf("TRUNCATE TABLE `homepage`");
                                    if($conn->query($sql_query) !== TRUE) {
                                        echo "Error 1: " . $conn->error;
                                        die();
                                    }

                                    $sql_query = sprintf("INSERT INTO `homepage`(`about`) VALUES ('%s')", $about);
                                    if($conn->query($sql_query) === TRUE) {
                                        header("Location: index.php");
                                    }
                                    else {
